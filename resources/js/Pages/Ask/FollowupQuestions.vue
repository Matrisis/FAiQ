...
const followupQuestion = (fu_question, fu_answer) => {
    emit("followupQuestion", {fu_question, fu_answer})
}
    ...
    <div class="h-full">
        ...
        <div v-else class="flex flex-col justify-between px-4 border-b border-r custom-border rounded-xl">
            <div  v-for="fuq in followupQuestions" class="flex py-4 flex-row items-center">
                ...
                <a @click="followupQuestion(fuq.question, fuq.answer)" class="flex hover:underline cursor-pointer font-bold text-lg text-color ">
                    {{ fuq.question }}
                </a>
            </div>
        </div>
